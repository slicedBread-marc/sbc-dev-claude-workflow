...
1. **Check branch** — if not on `develop`, save current branch and switch:
   ...
   ```
2. **Determine the next bug ID** — list `bugs/open/`, `bugs/triaged/`, and `bugs/closed/` to find the highest existing `BUG-NNN` number. Increment by 1. Start at `BUG-001` if none exist.
